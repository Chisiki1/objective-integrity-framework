...
def planned_files(adapter: str, mode: str = "minimal") -> dict[str, Path]:
    ...
    if mode == "complete":
        ...
        for name in ("README.md", "LICENSE", "PRIVACY.md"):
...
def rollback(backup: Path) -> int:
    ...
    if manifest.get("schema_version") != SCHEMA:
        raise ValueError("This backup lacks a v2 integrity inventory. Review and restore legacy backups manually.")
...
def plan_identity(destination: Path, adapter: str, mode: str, entries: list[dict]) -> str:
    plan = {"schema_version": "oif-preview-v1", "installer_sha256": digest(Path(__file__).read_bytes()),
            ...
            "adapter": adapter, "mode": mode, "entries": sorted(entries, key=lambda item: item["path"])}
    return digest(json.dumps(plan, sort_keys=True, separators=(",", ":")).encode("utf-8"))


def install(destination: Path, adapter: str, mode: str, apply: bool, expect_plan: str | None = None) -> int:
    ...
    plan_sha256 = plan_identity(destination, adapter, mode, entries)
    ...
    with install_lock(destination):
        ...
        manifest = {"schema_version": SCHEMA, "destination": str(destination), "adapter": adapter,
                    ...
                    "state": "PREPARING", "entries": entries, "applied": []}
        for entry in entries:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--rollback", help="Recover from a generated v2 backup; refuse later user edits.")
    args = parser.parse_args(argv)
    try:
        if args.rollback:
            if args.destination or args.apply or args.expect_plan:
                parser.error("--rollback cannot be combined with --destination, --apply or --expect-plan")
        ...
        return install(destination, args.adapter, args.mode, args.apply, args.expect_plan)
