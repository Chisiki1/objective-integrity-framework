...
1. Read the configured project and shared learning sources. Freeze the source-bound objective, every source-clause disposition, authority, scope, and exact finalized job, action, and tool facts.
   When a master carries `master-history-v1`, use the sibling lifecycle's archive-aware `master_index.py` or read the exact linked backing before claiming complete applicable history. Root-only search is incomplete. `inventory_live_masters_v2.py` follows the same protocol for structural ID/link coverage, not semantic supersession.
